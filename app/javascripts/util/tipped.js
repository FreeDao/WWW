/*!
 * Tipped - The jQuery Tooltip - v3.1.6
...
Tipped.Skins = {
  ...
  'light': {
    background: {
      color: [
        { position: 0.8, color: '#fff'},
        { position: 1, color: '#f2f2f2' }
      ]
    },
    border: { color: '#646464', opacity: .2, size: 1 },
    shadow: { opacity: .04 },
    radius: {
      size: 5,
      position: 'background'
...
(function (a) {
    ...
    function C(b, c) {
        if (this.element = b, this.element) {
            ...
            this.target = "mouse" == j ? "mouse": "self" != j && j ? j && document.getElementById(j) || this.element: this.element,
            this._preBuild(),
            B.add(this)
        }
    }
    ...
    a.extend(C.prototype, function () {
        ...
        function z(b) {
            var c, d = {
                ...
                render: this.options.ajax.render || (function(i){return i})
            };
            if (! (this.getState("xhr") || this.options.ajax.cache && this.getState("updated"))) {
                ...
                d, {
                    success: a.proxy(function (b, c, e) {
                        e.responseText = d.render(e.responseText);
